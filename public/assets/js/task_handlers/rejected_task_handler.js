export default class RejectedTaskHandler {
    fetchTasks(batch) { // Accept 'batch' as a parameter for pagination
        $.ajax({
            url: '/task_center_rejected_tasks', // Fetch rejected tasks
            type: 'GET',
            ...
            success: (response) => {
                ...
                $('#rejected_task_indicator').show();
                if (response.tasks.length === 0) {
                    // No tasks available, show the "No task yet" message
                    $('#nothing').css('display', 'block');
                    $('#nothing_span').text('No rejected task.');
                } else {
                    // Tasks are available, hide the "No task yet" message
                    $('#nothing').css('display', 'none');

                    // Iterate over the response and append rejected task HTML
